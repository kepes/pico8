...
**Goal:** Egy önálló, tesztelt PICO-8 cart (`carts/jedi/jedi.p8`): felülnézetes jedi-túlélő játék sivatagban, blokkolható és visszaverhető lézerlövésekkel, egyre több birodalmi katonával.

**Architecture:** Egyetlen `.p8` fájl (kód + gfx + sfx + music). A kód tab-okra bontva (main / world / player / troopers / bolts / fx-hud), globális `g` állapottáblával, OOP nélkül. A tesztek egy külön cart-ban (`test_jedi.p8`) élnek, ami `#include jedi.p8`-tal behúzza a kódot, stubolja a `btn`/`btnp`-t, és headless (`pico8 -x`) fut.
...
**Spec:** `carts/jedi/docs/superpowers/specs/2026-09-16-desert-jedi-design.md` — a terv a spec-ből érvel; a végrehajtó MINDKETTŐT olvassa.
...
|---|---|---|
| `carts/jedi/jedi.p8` | a játék, önálló cart | Agent A (kód), B (gfx), C (sfx/music), D (összefűzés), F (javítás) |
| `carts/jedi/test_jedi.p8` | headless tesztek, `#include jedi.p8` | Agent A, F |
| `carts/jedi/run_tests.sh` | tesztek + token-limit + lint futtató | Agent A |
| `carts/jedi/README.md` | leírás, irányítás, futtatás, teszt | Agent G |
| `carts/jedi/src/gfx.txt` | ideiglenes: 128 sor gfx hex (Task 4 után törlendő) | Agent B |
| `carts/jedi/src/sfx.txt`, `src/music.txt` | ideiglenes: 64+64 sor (Task 4 után törlendő) | Agent C |
...
|---|---|---|---|
| 1 | `carts/jedi` | Task 1: játékkód (`jedi.p8` `__lua__`), `test_jedi.p8`, `run_tests.sh`; tesztek zölden | **A** (Build-csoport, párhuzamos) |
...
| 4 | `carts/jedi` | Task 4: szekciók összefűzése `jedi.p8`-ba, `src/` törlése, `run_tests.sh` zöld | **D** (1–3 után) |
...
**Files:**
- Create: `carts/jedi/jedi.p8` (csak `__lua__` szekció; gfx/sfx/music később)
- Create: `carts/jedi/test_jedi.p8`
- Create: `carts/jedi/run_tests.sh`
...
- [ ] **Step 1: Teszt-harness váza** — `test_jedi.p8`:
...
rc=0
echo "== headless tests =="
out=$(perl -e 'alarm 90; exec @ARGV' -- "$P8" -x test_jedi.p8 2>&1); code=$?
echo "$out"
...
- [ ] **Step 4: `jedi.p8` main tab** — konstansok (spec 14.), `g`, segédek, `_init/_update/_draw`, `new_game`, `upd_play`, `elapsed`, `max_alive`.
...
**Files:**
- Create: `carts/jedi/src/gfx.txt` — pontosan 128 sor × 128 hex kisbetűs karakter.
...
**Files:**
- Create: `carts/jedi/src/sfx.txt` — 64 sor × 168 karakter.
- Create: `carts/jedi/src/music.txt` — 64 sor `FF AABBCCDD`.
...
**Files:** Modify `carts/jedi/jedi.p8`; delete `carts/jedi/src/`.

- [ ] **Step 1:** Python egysorossal: a `jedi.p8` `__lua__` szekcióját (a `__gfx__`/`__sfx__`/`__music__` fejlécig vagy a fájl végéig) megtartva írd újra a fájlt: `__lua__` + kód + `__gfx__` + `src/gfx.txt` + `__sfx__` + `src/sfx.txt` + `__music__` + `src/music.txt` (+ záró újsor).
- [ ] **Step 2:** `bash run_tests.sh` → `ALL GREEN`; `pico8 -x jedi.p8` időkorláttal (`alarm 10`) → csak `RUNNING:` sor, nincs `syntax error`/`runtime error` (a timeout miatti kilépés normális, mert a játék nem áll le magától).
- [ ] **Step 3:** `rm -r src/`.

## Task 5: Verifikáció (Agent E1–E5, párhuzamos)

Mind az öt agent **csak olvas/futtat** a `jedi/` mappában (E2/E5 saját segéd-cartja a scratchpadben: a `#include` **csak a cart mellé, relatív útvonalon** működik — abszolút út ellenőrzötten NEM megy —, ezért másold a `jedi.p8`-at a scratchpad-mappádba a segéd-cart mellé, és onnan `#include jedi.p8`). Kimenet: strukturált findings lista `{severity: high|medium|low, area, description, evidence, suggested_fix}` + rövid összefoglaló. **high** = spec-sértés, futásidejű hiba, tesztek pirosak, limit-túllépés, felismerhetetlen sprite; **medium** = hangolási/érzet-probléma, hiányzó teszteset, olvashatatlan HUD; **low** = kozmetika.

- **E1 (tesztek):** `bash run_tests.sh`; a kimenet alapján findings (piros teszt = high; token > 7000 = medium figyelmeztetés).
- **E2 (képernyőképek):** scratchpad `shots.p8`: `#include` a játék **és a `jedi.p8` `__gfx__`/`__sfx__`/`__music__` szekciói a segéd-cart végére másolva** (a `#include` csak kódot hoz; e nélkül üres a sprite-lap), stub `btn/btnp` mint a harnessben, `_init`: `srand(3) new_game()`; frame-vezérelt szkript a saját `_update`-jében: 1) `g.state="title"` → 3 frame → `extcmd("screen")`; 2) `g.state="play"`, majd 600 frame lépés úgy, hogy 2–3 katona legyen (`g.frames=1200`-ra állítva a `max_alive` miatt, spawn kikényszerítve `spawn_trooper()` ×3-mal), közben ❎ nyomva, a jedi jobbra néz → screen; 3) suhintás közben (🅾️ lenyomás után 3 frame) → screen; 4) `g.p.hp=1` + lövedék a jedibe → dying → 60 frame → game over → screen. Futtatás: `pico8 -desktop <scratch/shots> -x shots.p8`. A PNG-ket **nézd meg** (Read), és a spec 8., 10., 15.3 ellen értékelj: felismerhető jedi (barna+kék kard), katonák (fehér), sziklák, homok, HUD szívek/pontszám olvasható, cím szöveg középen, game over doboz.
- **E3 (spec-megfelelés, adverzáriális):** olvasd a spec 3–7. és 16. szakaszát, majd a `jedi.p8` kódot, és **keress eltérést** minden számszerű szabálynál (kúpok, távolságok, időzítők, `max_alive`, spawn, `stop` képlet, dead 90 frame, pontszám 10/20, blokk pozíció-alapú kúp, inv alatt blokk működik, katonák nem sebeznek érintéssel, lövedék sziklán megsemmisül). Minden eltérés = high, hivatkozott sorral.
...
- [ ] **Step 2:** Minden high és medium tételt javíts; ahol a találat tesztelhető, **előbb írj rá tesztet** a `test_jedi.p8`-ba (piros → zöld).
...
**Files:** Create `carts/jedi/README.md` (magyar).

- [ ] Tartalom: mi ez (2 mondat), **irányítás** táblázat, **futtatás** PICO-8-ban (`load jedi/jedi.p8` → `run`, vagy splore), **játékmenet** röviden (blokk-kúp, visszaverés, pontok 10/20, nehézség 20 mp-enként +1 katona), **tesztelés**: `bash run_tests.sh` — mit csinál (headless tesztek, shrinko8 token-limit, lint), előfeltételek (PICO-8 az `/Applications`-ben, `uv`), a `test_jedi.p8` szerkezete (harness + `tcase()/check()/step()`), **fájlok** lista, hivatkozás a spec-re és a tervre. Ha > 3 H2 → tartalomjegyzék a bevezető után.
...
Spec: <dir>/docs/superpowers/specs/2026-09-16-desert-jedi-design.md
Plan: <dir>/docs/superpowers/plans/2026-09-16-desert-jedi-plan.md
...
- **A:** „Execute Task 1 (Agent A) end to end: write jedi.p8 (`__lua__` only), test_jedi.p8 with all 15 test cases from spec §12, run_tests.sh; iterate until `bash run_tests.sh` prints ALL GREEN. Keep tokens ≤ 6500 (shrinko8 --count). Draw calls must not error with an empty sprite sheet."
...
- **D:** „Execute Task 4 (Agent D): merge src/gfx.txt, src/sfx.txt, src/music.txt into jedi.p8 after the `__lua__` code, verify with run_tests.sh and a 10 s headless run of jedi.p8, then delete src/."
...
4. **Docs → kész:** `run_tests.sh` ALL GREEN; a `jedi/` mappában csak `jedi.p8`, `test_jedi.p8`, `run_tests.sh`, `README.md`, `docs/`.
